...
# ساخت کیبورد ثابت پایین صفحه (Reply Keyboard)
...
# هندلر برای دکمه‌های ثابت پایین صفحه (متنی)
...
def handle_user_receipt(message):
    ...
    # اگر پیام جزو دکمه‌های منوی اصلی بود، نادیده بگیر تا تداخل نکند
...
def handle_clear_warehouse(call):
    ...
    markup = InlineKeyboardMarkup()
    for c in categories:
        count = get_stock_count(c)
        p_name = plan_details[c]['name']
        markup.add(InlineKeyboardButton(f"➕ افزودن {p_name} ({count} عدد)", callback_data=f"add_{c}"))
        markup.add(InlineKeyboardButton(f"🗑 خالی کردن انبار {p_name}", callback_data=f"clear_{c}"))
    markup.add(InlineKeyboardButton("💳 تغییر شماره کارت بانکی", callback_data="edit_card"))
    
    text_msg = "⚙️ **پنل مدیریت انبارها و کارت**\n\n📊 **موجودی انبارها:**\n"
    for c in categories:
        text_msg += f"- {plan_details[c]['name']}: `{get_stock_count(c)}` کانفیگ\n"
    text_msg += f"\n💳 **شماره کارت فعلی:**\n`{current_card}`"
...
def ask_for_config(call):
    ...
    msg = bot.send_message(message := call.message, f"کانفیگ‌های مربوط به **{plan_details[cat]['name']}** را بفرستید (هر خط یک کانفیگ):", parse_mode="Markdown")
