...
def get_main_markup(user_id):
    ...
    markup.add(InlineKeyboardButton("⚡ خرید کانفیگ (اقتصادی / حرفه‌ای)", callback_data="buy_menu"))
...
def admin_panel(call):
    ...
    text = "⚙️ **پنل مدیریت انبارها و کارت**\n\n📊 **موجودی انبارها:**\n"
    for cat in categories:
        text += f"- {plan_details[cat]['name']}: `{get_stock_count(cat)}` کانفیگ\n"
    text += f"\n💳 **شماره کارت فعلی:**\n`{current_card}`"
