...
def ask_for_config(call):
    ...
    plans = load_plans()
    p_name = plans.get(cat, {}).get("name", cat)
    msg = bot.send_message(message.chat.id if hasattr(message, 'chat') else call.message.chat.id, f"کانفیگ‌های مربوط به **{p_name}** را بفرستید (هر خط یک کانفیگ):", parse_mode="Markdown")
