...
# مدیریت سفارشات و ساب‌سکریپشن‌های کاربران
...
def get_user_account_info(user_id):
    ...
    if not user_configs:
        tariffs_text = "\n".join([f"🔹 {info['name']}: **{info['price']}**" for info in plan_details.values()])
        return (
    ...
    for idx, item in enumerate(user_configs, 1):
        ...
        p_name = plan_details.get(cat, {}).get("name", "سرویس")
        purchase_date_str = item.get("purchase_date")
        
        try:
            p_date = datetime.fromisoformat(purchase_date_str)
            expiry_date = p_date + timedelta(days=30)  # فرض اعتبار ۳۰ روزه برای هر کانفیگ
...
def admin_action(call):
    ...
    if action == "approve":
        ...
        # ذخیره سفارش برای پنل حساب کاربری کاربر
