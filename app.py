...
# ساختار جامع شامل هم پلن‌های ثابت و هم پلن‌های حجمی گیگابایتی
DEFAULT_DATA = {
    "price_per_gb": 25000,
    "fixed_plans": {
...
def get_plan_info(category):
    ...
    if category in data.get("fixed_plans", {}):
        p = data["fixed_plans"][category]
        return p["name"], p["price"]
    elif category in data.get("vol_tiers", {}):
        p = data["vol_tiers"][category]
        price_per_gb = data.get("price_per_gb", 25000)
        calc_price = p["gb"] * price_per_gb
        return p["name"], format_price(calc_price)
    return "سرویس", "0 تومان"
...
def get_user_account_info(user_id):
    ...
    if not user_configs:
        ...
        tariffs_text += f"\n🌐 **پکیج‌های حجمی (قیمت هر گیگ: {format_price(price_per_gb)}):**\n"
...
def get_admin_panel_data():
    ...
    markup.add(InlineKeyboardButton("📌 --- مدیریت پکیج‌های حجمی (گیگی) ---", callback_data="none"))
    ...
    text_msg = (
        f"⚙️ **پنل مدیریت جامع (ثابت + حجمی)**\n\n"
        f"💵 قیمت هر گیگابایت: **{format_price(price_per_gb)}**\n\n"
        f"📊 **پکیج‌های ثابت:**\n"
    )
    for cat, info in fixed_plans.items():
    ...
    for cat, info in vol_tiers.items():
        ...
        cnt = get_stock_count(cat)
        cnt_str = f"`{cnt} عدد`" if cnt > 0 else "⚠️ `خالی`"
        text_msg += f"- {info['name']}: موجودی: {cnt_str} | قیمت اتوماتیک: `{format_price(p_calc)}`\n"
    ...
    if text == "⚡ خرید کانفیگ":
        ...
        if fixed_plans:
            ...
            for cat, info in fixed_plans.items():
                ...
                markup.add(InlineKeyboardButton(f"📦 {info['name']} — {info['price']} ({stock_label})", callback_data=f"buyplan_{cat}"))
                
        if vol_tiers:
            markup.add(InlineKeyboardButton("📌 --- پکیج‌های حجمی (گیگی) ---", callback_data="none"))
            for cat, info in vol_tiers.items():
                count = get_stock_count(cat)
                p_calc = info['gb'] * price_per_gb
                stock_label = f"موجودی: {count}" if count > 0 else "⚠️ ثبت سفارشی"
        ...
        bot.send_message(message.chat.id, "لطفاً پکیج یا حجم مورد نظر خود را انتخاب کنید:", reply_markup=markup)
...
def show_price_and_confirm(call):
    ...
    p_name, p_price = get_plan_info(category)
...
def handle_user_receipt(message):
    ...
    p_name, _ = get_plan_info(category)
...
def admin_action(call):
    ...
    if action == "approve":
        ...
        if not config:
            # انبار خالی است -> درخواست لینک دستی از ادمین
...
def ask_vol_plan_key(call):
    ...
    if not is_admin(call.from_user.id):
        return
    msg = bot.send_message(call.message.chat.id, "یک شناسه انگلیسی کوتاه برای پکیج حجمی بفرستید (مثلاً: `vol_30`):", parse_mode="Markdown")
...
def finalize_vol_plan(message, key, name):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"✅ پکیج حجمی **{name}** با قیمت خودکار **{format_price(calc_price)}** ساخته شد!")
    except ValueError:
