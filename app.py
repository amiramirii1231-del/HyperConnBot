...
def ask_for_config(call):
    ...
    msg = bot.send_message(call.message.chat.id, f"لطفاً کانفیگ خامِ مورد نظر را بفرستید (می‌توانید چند خطی بفرستید):", parse_mode="Markdown")
...
def save_config_step(message, category):
    ...
    for line in text.split("\n"):
        if line.strip():
            ...
            count += 1
    bot.send_message(message.chat.id, f"✅ تعداد {count} کانفیگ با موفقیت به انبار اضافه شدند!")
