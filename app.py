...
def ensure_warehouse_files():
    ...
    for cat in all_cats:
        fname = f"{cat}.txt"
        if not os.path.exists(fname): open(fname, "w", encoding="utf-8").close()

ensure_warehouse_files()

if not os.path.exists("card.txt"): open("card.txt", "w", encoding="utf-8").write("ثبت نشده")
...
def save_user_order(user_id, category, config):
    ...
    orders[str(user_id)].append({"category": category, "config": config, "date": datetime.now().isoformat()})
    with open("user_orders.json", "w", encoding="utf-8") as f:
        json.dump(orders, f, ensure_ascii=False, indent=4)

def format_price(amount): return f"{amount:,} تومان" if isinstance(amount, (int, float)) else str(amount)
...
def get_plan_info(full_cat):
    ...
    if base_cat.startswith("custom_"):
        try:
            ...
            base_p = gb * price_gb
            return f"{gb} گیگابایت ({users} کاربره)", format_price(base_p + extra_cost), format_price(base_p), format_price(extra_cost), users
        except: return "نامشخص", "0 تومان", "0 تومان", "0 تومان", users
        ...
        return f"{p['name']}{u_suffix}", format_price(base_p + extra_cost), format_price(base_p), format_price(extra_cost), users
        
    elif base_cat in data.get("vol_tiers", {}):
        p = data["vol_tiers"][base_cat]
        base_p = p["gb"] * price_gb
        return f"{p['name']} ({users} کاربره)", format_price(base_p + extra_cost), format_price(base_p), format_price(extra_cost), users
...
def main_menu_handler(message):
    ...
    if text == "⚡ خرید کانفیگ":
        ...
        if not orders:
            bot.send_message(message.chat.id, "شما هنوز خریدی نداشته‌اید.")
        else:
    ...
    # ===== بخش پنل ادمین (که در نسخه قبل جا افتاده بود) =====
    elif text == "⚙️ پنل مدیریت ادمین":
        if user_id == ADMIN_ID:
            markup = InlineKeyboardMarkup(row_width=1)
            markup.add(InlineKeyboardButton("💳 تنظیم شماره کارت", callback_data="admin_card"))
            markup.add(InlineKeyboardButton("📦 مدیریت انبار کانفیگ", callback_data="admin_add_stock"))
            markup.add(InlineKeyboardButton("📊 آمار فروش", callback_data="admin_stats"))
            bot.send_message(message.chat.id, "🛠 **پنل مدیریت ادمین:**\nبخش مورد نظر را انتخاب کنید:", reply_markup=markup, parse_mode="Markdown")
        else:
            bot.send_message(message.chat.id, "⛔️ شما دسترسی به این بخش را ندارید.")

# ==========================================
# دکمه‌های شیشه‌ای (Callbacks) با سیستم ضد کرش و متغیرهای دقیق
# ==========================================
...
def callback_handler(call):
    ...
    try:
        ...
        if data == "buy_menu":
            ...
            call.message.text = "⚡ خرید کانفیگ"
            main_menu_handler(call.message)
            return

        # --- دکمه‌های پنل ادمین ---
        if data == "admin_card":
            if call.from_user.id != ADMIN_ID: return
            msg = bot.edit_message_text("💳 لطفاً شماره کارت جدید را ارسال کنید:", chat_id=chat_id, message_id=msg_id)
            ...
            markup = InlineKeyboardMarkup(row_width=1)
            data_json = load_data()
            for cat, info in data_json.get("fixed_plans", {}).items():
                markup.add(InlineKeyboardButton(f"📦 افزودن: {info['name']}", callback_data=f"stk_{cat}"))
            for cat, info in data_json.get("vol_tiers", {}).items():
                markup.add(InlineKeyboardButton(f"🌐 افزودن: {info['name']}", callback_data=f"stk_{cat}"))
            markup.add(InlineKeyboardButton("❌ بازگشت", callback_data="buy_menu"))
            bot.edit_message_text("📦 برای کدام پکیج می‌خواهید کانفیگ اضافه کنید؟", chat_id=chat_id, message_id=msg_id, reply_markup=markup)

        elif data.startswith("stk_"):
            if call.from_user.id != ADMIN_ID: return
            cat = data.split("stk_", 1)[1]
            msg = bot.edit_message_text("لطفاً کانفیگ‌ها را ارسال کنید (هر کانفیگ در یک خط مجزا باشد):", chat_id=chat_id, message_id=msg_id)
            bot.register_next_step_handler(msg, admin_add_stock, cat)

        elif data == "admin_stats":
            if call.from_user.id != ADMIN_ID: return
            orders = load_user_orders()
            total_orders = sum(len(user_orders) for user_orders in orders.values())
            txt = f"📊 **آمار سیستم:**\n\nتعداد کل مشتریان: {len(orders)}\nتعداد کل خریدها: {total_orders}"
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("❌ بستن", callback_data="buy_menu"))
        ...
        # --- 1. انتخاب ساخت حجم دلخواه ---
            ...
            msg = bot.edit_message_text("تعداد گیگابایت مورد نظرتان را وارد کنید (فقط عدد، مثلا 30):", chat_id=chat_id, message_id=msg_id)
        ...
        # --- 2. انتخاب یکی از پلن‌های ثابت یا حجمی ---
            ...
            bot.edit_message_text("سرویس انتخاب شد.\nحالا لطفاً تعداد کاربر همزمان را انتخاب کنید:", chat_id=chat_id, message_id=msg_id, reply_markup=markup)

        # --- 3. انتخاب تعداد کاربر ---
        ...
        # --- 4. نمایش شماره کارت ---
            ...
            msg = bot.edit_message_text(text, chat_id=chat_id, message_id=msg_id, parse_mode="Markdown", reply_markup=markup)
        ...
        # --- 5. ادمین تایید/رد فیش ---
            ...
            if action == "rej":
                ...
                try: bot.edit_message_caption("❌ رد شد.", chat_id=chat_id, message_id=msg_id)
                except: bot.edit_message_text("❌ رد شد.", chat_id=chat_id, message_id=msg_id)
                ...
                if config:
                    ...
                    try: bot.edit_message_caption("✅ تایید شد (تحویل خودکار از انبار).", chat_id=chat_id, message_id=msg_id)
                    except: bot.edit_message_text("✅ تایید شد (تحویل خودکار از انبار).", chat_id=chat_id, message_id=msg_id)
                else:
                    try: bot.edit_message_caption("⏳ منتظر ارسال دستی کانفیگ...", chat_id=chat_id, message_id=msg_id)
                    except: bot.edit_message_text("⏳ منتظر ارسال دستی کانفیگ...", chat_id=chat_id, message_id=msg_id)
    ...
    # سیستم گرفتن مچ ارورها تا بات هنگ نکنه
    except Exception as e:
        bot.send_message(chat_id, f"❌ خطای سرور رخ داد. لطفا دوباره تست کنید. (کد ارور: {e})")
...
def admin_set_card(message):
    ...
    bot.send_message(message.chat.id, "✅ شماره کارت با موفقیت ثبت شد.")

def admin_add_stock(message, cat):
    if message.text in ["⚡ خرید کانفیگ", "👤 حساب کاربری و تعرفه‌ها", "📞 پشتیبانی HyperConn", "⚙️ پنل مدیریت ادمین"]:
        return main_menu_handler(message)
    ...
    if configs:
        ...
        bot.send_message(message.chat.id, f"✅ تعداد {len(configs)} کانفیگ به انبار اضافه شد.")
    else:
        bot.send_message(message.chat.id, "❌ خطایی رخ داد، کانفیگی یافت نشد.")
...
def process_receipt(message, full_cat):
    ...
    try:
        if message.photo: bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=caption, parse_mode="Markdown", reply_markup=admin_markup)
        else: bot.send_message(ADMIN_ID, caption + f"\n\n💬 متن ارسالی:\n{message.text}", parse_mode="Markdown", reply_markup=admin_markup)
