...
# مدیریت داینامیک پلن‌ها، قیمت‌ها و نام‌ها در prices.json
DEFAULT_PLANS = {
...
def load_plans():
    if not os.path.exists("prices.json"):
        with open("prices.json", "w", encoding="utf-8") as f:
            json.dump(DEFAULT_PLANS, f, ensure_ascii=False, indent=4)
...
# مطمئن شدن از وجود فایل‌های انبار برای پلن‌های پیش‌فرض
def ensure_warehouse_files():
    plans = load_plans()
    for cat in plans.keys():
        fname = f"{cat}.txt"
...
def get_user_account_info(user_id):
    ...
    if not user_configs:
        tariffs_text = "\n".join([f"🔹 {info['name']}: **{info['price']}**" for info in plans.values()])
        return (
            "👤 **اطلاعات حساب کاربری شما**\n\n"
            "شما تاکنون هیچ کانفیگی خریداری نکرده‌اید.\n\n"
            "📋 **تعرفه‌های فعال پکیج‌ها:**\n" + tariffs_text
        )
    ...
    for idx, item in enumerate(user_configs, 1):
        ...
        p_name = plans.get(cat, {}).get("name", "سرویس حذف شده")
...
def get_admin_panel_data():
    ...
    for cat, info in plans.items():
        count = get_stock_count(cat)
        p_name = info['name']
        p_price = info['price']
        markup.add(InlineKeyboardButton(f"➕ افزودن به {p_name} ({count} عدد)", callback_data=f"add_{cat}"))
        markup.add(InlineKeyboardButton(f"✏️ تغییر نام پلن", callback_data=f"editname_{cat}"))
        markup.add(InlineKeyboardButton(f"💰 تغییر قیمت ({p_price})", callback_data=f"editprice_{cat}"))
        markup.add(InlineKeyboardButton(f"🗑 خالی کردن انبار", callback_data=f"clear_{cat}"))
        markup.add(InlineKeyboardButton(f"❌ حذف کامل این پلن", callback_data=f"deleteplan_{cat}"))
        markup.add(InlineKeyboardButton("➖➖➖➖➖➖➖➖➖➖", callback_data="none"))
        
    markup.add(InlineKeyboardButton("➕ ساخت و افزودن پلن جدید", callback_data="create_new_plan"))
    ...
    text_msg = "⚙️ **پنل مدیریت پیشرفته پلن‌ها، قیمت‌ها و انبار**\n\n📊 **وضعیت پلن‌ها:**\n"
    for cat, info in plans.items():
        text_msg += f"- {info['name']}: موجودی `{get_stock_count(cat)}` | قیمت: `{info['price']}`\n"
    text_msg += f"\n💳 **شماره کارت فعلی:**\n`{current_card}`"
...
def handle_reply_buttons(message):
    ...
    if text == "⚡ خرید کانفیگ":
        ...
        for cat, info in plans.items():
            count = get_stock_count(cat)
            markup.add(InlineKeyboardButton(f"{info['name']} — {info['price']} (موجودی: {count})", callback_data=f"buyplan_{cat}"))
...
def show_price_and_confirm(call):
    ...
    if get_stock_count(category) <= 0:
        bot.answer_callback_query(call.id, "❌ متأسفانه موجودی این پلن در حال حاضر تمام شده است.", show_alert=True)
        return

    plans = load_plans()
    p_info = plans.get(category, {})
...
def admin_action(call):
    ...
    if action == "approve":
        ...
        plans = load_plans()
        if not config:
            bot.answer_callback_query(call.id, f"❌ انبار این پلن ({plans.get(category, {}).get('name', category)}) خالی است!", show_alert=True)
            return
...
def save_new_name_step(message, category):
    ...
    if category in plans:
        plans[category]["name"] = new_name
        save_plans(plans)
        bot.send_message(message.chat.id, f"✅ نام پلن با موفقیت به **{new_name}** تغییر یافت!")
    else:
        bot.send_message(message.chat.id, "❌ پلن مورد نظر یافت نشد.")
...
def ask_for_new_price(call):
    ...
    plans = load_plans()
    p_name = plans[cat]["name"]
    msg = bot.send_message(call.message.chat.id, f"لطفاً قیمت جدید برای **{p_name}** را وارد کنید (مثلاً: `320,000 تومان`):", parse_mode="Markdown")
...
def save_new_price_step(message, category):
    ...
    plans = load_plans()
    if category in plans:
        plans[category]["price"] = new_price_text
        save_plans(plans)
        bot.send_message(message.chat.id, f"✅ قیمت **{plans[category]['name']}** با موفقیت به `{new_price_text}` تغییر یافت!", parse_mode="Markdown")
    else:
        bot.send_message(message.chat.id, "❌ خطایی رخ داد، پلن مورد نظر یافت نشد.")
...
def delete_plan(call):
    ...
    plans = load_plans()
    if len(plans) <= 1:
        bot.answer_callback_query(call.id, "❌ حداقل یک پلن باید باقی بماند!", show_alert=True)
        return
    
    p_name = plans.get(cat, {}).get("name", cat)
    plans.pop(cat, None)
    save_plans(plans)
    
    # حذف فایل انبار مربوطه اگر وجود داشت
    try:
    ...
    bot.answer_callback_query(call.id, f"✅ پلن {p_name} به طور کامل حذف شد.", show_alert=True)
    text_msg, markup = get_admin_panel_data()
    try:
        bot.edit_message_text(text_msg, call.message.chat.id, call.message.message_id, parse_mode="Markdown", reply_markup=markup)
    except Exception:
...
def ask_new_plan_key(call):
    ...
    if not is_admin(call.from_user.id):
        return
    msg = bot.send_message(call.message.chat.id, "یک شناسه انگلیسی و کوتاه برای پلن جدید بفرستید (مثلاً: `vip_single` یا `special_plan`):", parse_mode="Markdown")
...
def ask_new_plan_name(message):
    ...
    if key in plans:
        ...
        return
    msg = bot.send_message(message.chat.id, f"حالا نام نمایشی پلن را بفرستید (مثلاً: `پکیج VIP - ۱ کاربره`):", parse_mode="Markdown")
    bot.register_next_step_handler(msg, ask_new_plan_price, key)

def ask_new_plan_price(message, key):
    if not is_admin(message.from_user.id):
        return
    name = message.text.strip()
    msg = bot.send_message(message.chat.id, f"حالا قیمت پلن را بفرستید (مثلاً: `450,000 تومان`):", parse_mode="Markdown")
...
def finalize_new_plan(message, key, name):
    ...
    plans = load_plans()
    plans[key] = {"name": name, "price": price}
    save_plans(plans)
    ensure_warehouse_files()
    
    bot.send_message(message.chat.id, f"✅ پلن جدید **{name}** با قیمت **{price}** با موفقیت ساخته شد و به لیست خرید کاربران اضافه گردید!")

# --- افزودن کانفیگ به انبار پلن ---
...
def save_config_step(message, category):
    ...
    plans = load_plans()
    p_name = plans.get(category, {}).get("name", category)
    count = 0
