...
def ask_user_count(call):
    ...
    p_name, base_price_str, _, _, _ = get_plan_info(base_cat)
    text = (
    ...
    if action == "approve":
        ...
        if not config:
            ...
            msg = bot.send_message(
                ...
                f"⚠️ **سفارش برای پکیج ({p_name})**\n\nلطفاً لینک کانفیگ یا ساب‌مکرایب اختصاصی ساخته شده را برای این کاربر ارسال کنید (همینجا متن یا لینک را بفرستید):",
                parse_mode="Markdown"
            )
