...
# تابعی برای خنثی کردن کاراکترهای حساس مارک‌داون در اسم و یوزرنیم
def clean_md(text):
    if not text:
        return ""
...
def handle_user_messages(message):
    ...
    # اعمال تابع پاکسازی روی اسم و یوزرنیم کاربر برای جلوگیری از کرش کردن ربات
    ...
    try:
        if message.photo:
            ...
            safe_text_msg = clean_md(message.text)
...
def admin_action(call):
    ...
    if action == "approve":
        ...
        if not config:
            # موجودی صفر است - درخواست کانفیگ دستی
        ...
        # موجودی در انبار هست - ارسال خودکار
